  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
      <main className="container max-w-6xl px-4 lg:px-8 pt-6 lg:pt-10 space-y-5">
        <PageHeader title={`Pedido #${id}`} subtitle={job.service} back />
        {/* Pro card */}
        {status !== "pending" && status !== "cancelled" && (
          <section className="relative overflow-hidden rounded-3xl bg-gradient-hero p-5 shadow-glow text-primary-foreground">
            <div className="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-white/15 blur-2xl" />
            <div className="relative flex items-center gap-4">
              <div className="h-14 w-14 rounded-2xl bg-white/20 backdrop-blur flex items-center justify-center text-lg font-extrabold">
                {job.initials}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs opacity-90">Profissional</p>
                <p className="font-extrabold text-lg truncate">{job.pro}</p>
                <p className="text-xs inline-flex items-center gap-1 opacity-90">
                  <Star className="h-3 w-3 fill-current" /> {job.rating} ·{" "}
                  {job.service}
                </p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2">
              <Button
                size="sm"
                variant="outline"
                className="bg-white/15 border-white/30 text-primary-foreground hover:bg-white/25 hover:text-primary-foreground">
        ...
        <section className="rounded-3xl bg-card border border-border/60 shadow-card p-5 space-y-3">
          ...
          <div className="flex items-center justify-between text-sm pt-6 lg:pt-10 border-t border-border/60">
            <span className="text-muted-foreground">Preço acordado</span>
            <span className="font-extrabold text-base">{job.price}</span>
          </div>
        </section>
